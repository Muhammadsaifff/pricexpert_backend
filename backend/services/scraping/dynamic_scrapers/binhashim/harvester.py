...
API_HEADERS = {
    ...
    "Referer": f"{BASE_URL}/",
    "Origin": BASE_URL,
    "app-name": "binhashimpharmacysupermarket",
    "rest-id": REST_ID,
}
...
def _get_session_cookies():
    """
    Visit Bin Hashim website with Selenium to get valid session cookies.
    """
    options = Options()
...
def harvest_subsection(session, sub_section_id, section_name="Unknown", per_page=24):
    ...
    while page <= max_pages:
        ...
        try:
            ...
            print(f"      [BinHashim] {section_name} | Page {page}: {len(products)} products")
            ...
            print(f"      [BinHashim] Error on page {page}: {e}")
            break
...
def start_harvest(categories, workers=3):
    """
    Main entry point for Bin Hashim harvester.
    """
    print("   [BinHashim] Bootstrapping session...")
    ...
    print(f"   [BinHashim] Harvesting {len(normalized)} sub-sections...")
    all_products = []
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                all_products.extend(products)
                print(f"   [BinHashim] Done: {cat['name']}: {len(products)} products")
            except Exception as e:
                print(f"   [BinHashim] {cat['name']} failed: {e}")

    # Deduplicate by product ID
    unique = {p.get('id', p.get('name')): p for p in all_products}
    results = list(unique.values())

    print(f"\n   [BinHashim] Harvest complete: {len(results)} unique products")

    return [{**p, "url": f"{BASE_URL}/product/{p.get('slug', p.get('id'))}"} for p in results]
